...
##parameters=q,limit=30,path=None
...
url_tokens = context.REQUEST.getURL().split('/')
...
USE_ICON = True
USE_RANKING = False
if 'preiskave' in url_tokens: 
  MAX_TITLE = 100
elif len(url_tokens) == 4: 
  MAX_TITLE = 10000
else:
  MAX_TITLE = 29
...
if 'preiskave' in url_tokens or len(url_tokens) == 4: 
  limit = 20
  params = {'moj': r,
...
if not results:
    ...
    write('''<legend id="livesearchLegend">Live search</legend>''')
    write('''<div class="LSIEFix">''')
    write('''<div id="LSNothingFound">No match found</div>''')
    ...
    write('''<legend id="livesearchLegend">Live search results: %s</legend>''' % (len(results)))
    write('''<div class="LSIEFix">''')
    ...
    if len(results)>limit:
        ...
        write('''<span>prikazanih %s od %s zadetkov</span>''' % (limit, len(results)))
        if 'hitro-iskanje' in url_tokens: 
          write( '<a href="%s" style="font-weight:normal">%s</a>' % ('search?moj=' + searchterms, 'prikaži vse zadetke'))
        else:
